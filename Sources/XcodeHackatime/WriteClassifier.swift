...
/// and the user's activity in that file. Pure - no clock, no filesystem -
/// so the truth table is directly testable.
///
...
///
/// The two user-write rows encode how saves actually happen: ⌘S and
/// autosave land during or just after the user's editing in that file, and
/// a ⌘S right after a long pause shows up as a fresh mtime on a live event.
/// Everything else - git pull, a formatter, a generator, a tool preserving
/// historical timestamps, a future mtime from clock skew - is external.
enum WriteClassifier {
    /// ⌘S/autosave lands within this long of the user's editing in the file.
    static let saveSlack: TimeInterval = 60
    /// On a live user event, an mtime at most this old is the user's save
    /// even without recent per-file activity (return-from-break ⌘S). The
    /// -1s lower bound tolerates sub-second skew, nothing more.
    static let recentWriteWindow: TimeInterval = 10

    enum Verdict {
        /// No committed baseline yet: establish one; nothing to attribute.
        case baseline
        case unchanged
    ...
    static func classify(diskMTime: Date?, baselineMTime: Date?, lastActivity: Date?,
                         now: Date, userAction: Bool) -> Verdict {
        ...
        let duringActivity = lastActivity.map {
